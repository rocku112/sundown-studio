...
(function (P) {
  ...
  K('measurable', '限制可量測',
    ...
    function (c) { return /(不超過|至多|最多|至少|超過|介於|上限|下限|恰好)\s*[\d一二三四五六七八九十兩]/.test(c.text) || /\d+\s*(字|則|項|個|條|行)以[內下上]/.test(c.text); });
  ...
  K('docDelim', '多文件結構化分隔',
    ...
    function (c) {
      // 帶屬性的重複標籤（<文件 id="D1">、<案件 id="1">）或管線分隔的表頭都算
      return /<[\w一-鿿]+\s+(id|編號|標題|日期)\s*=/i.test(c.text)
        || /(<(doc|document|文件)[^>]*>[\s\S]*<\/(doc|document|文件)>)/i.test(c.text)
        || /\|\s*(來源|標題|ID)\s*\|/.test(c.text);
    });
  ...
  K('allowIDK', '允許回答不知道',
    ...
    function (c) {
      return /(不知道|不確定|無法確認|無法判斷|無法[^\n]{0,10}(找到|判斷|確認)|資料不足|資訊不足|資料未載|查無|找不到|未找到|未涵蓋|未載|未記載|未提及|未揭露)[^\n]{0,24}(就|請|直接|回答|回覆|說明|標註|標明|寫|填)/.test(c.text);
    });
  ...
  K('citeFormat', '指定引用格式',
    ...
    function (c) { return /(引用格式|標註\s*(為)?\s*[［\[]|後方?標註\s*[［\[]|以\s*[［\[][^\]］]*[\]］]\s*標註|來源以[^\n]{0,10}表示)/.test(c.text); });
  ...
  K('memoryFile', '常駐指令寫成檔案',
    ...
    function (c) { return /(AGENTS\.md|CLAUDE\.md|[A-Z]{4,}\.md|常駐指令|規則檔)/.test(c.text); });
